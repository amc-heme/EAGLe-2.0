...
gene.jordan.m0m5 <- t(as.data.frame(exp.jordan.m0m5)[-c(5:60162),])
...
navbarMenu("BEAT-AML",
        ...
        #                    choices = c("SampleID", "PercentBlastsInBM", "PercentBlastsInPB"),
        ...
        #     mainPanel(
        #        tabsetPanel(type = "tabs",
        #                    tabPanel("Plot", plotOutput("plotgraph1")),
        #                    tabPanel("Table", tableOutput("table")),
        #                    tabPanel("Summary",textOutput("summary")))
        #               ),
        #   ),
...
server <- function(input, output, session) {
 ...
 updateSelectizeInput(session,"gene_choice", choices = exp.jordan.m0m5$V2, server = TRUE) 
  ...
  output$plotgraph2 <- renderPlot({
    ...
    ggplot(qc, aes(metadata.sample_id, data2)) + 
      geom_point(size = 3) +
      theme_cowplot (12) +
      theme(axis.text.x =
  ...
  output$plotgraph1 <- renderPlot({
    ...
    print("data:")
    print(data)
    ggplot(meta_lut_ven, aes(AUC, data)) +
      geom_point() +
      theme_light() +
